...
//Surius:     Flag setters/getters are generated by macro for CF/OF/ZF/SF.
...
static inline void rtl_mv(rtlreg_t* dest, const rtlreg_t *src1) {
  // dest <- src1
  //Surius:     Move value between RTL temporaries.
  *dest = *src1;
}

static inline void rtl_not(rtlreg_t* dest) {
  // dest <- ~dest
  //Surius:     Bitwise NOT in place.
  *dest = ~(*dest);
}

static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  // dest <- signext(src1[(width * 8 - 1) .. 0])
  //Surius:     Sign-extend source according to operand width.
  switch (width) {
...
static inline void rtl_eq0(rtlreg_t* dest, const rtlreg_t* src1) {
  // dest <- (src1 == 0 ? 1 : 0)
  //Surius:     Compare against zero.
  *dest = (*src1 == 0);
}

static inline void rtl_eqi(rtlreg_t* dest, const rtlreg_t* src1, int imm) {
  // dest <- (src1 == imm ? 1 : 0)
  //Surius:     Compare against immediate constant.
  *dest = (*src1 == (rtlreg_t)imm);
}

static inline void rtl_neq0(rtlreg_t* dest, const rtlreg_t* src1) {
  // dest <- (src1 != 0 ? 1 : 0)
  //Surius:     Test non-zero.
  *dest = (*src1 != 0);
}

static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  // dest <- src1[width * 8 - 1]
  //Surius:     Extract sign bit at current width.
  *dest = (*src1 >> (width * 8 - 1)) & 0x1;
}

static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  // eflags.ZF <- is_zero(result[width * 8 - 1 .. 0])
  //Surius:     Update ZF using width-masked result.
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  ...
  //Surius:     Update SF from the sign bit.
